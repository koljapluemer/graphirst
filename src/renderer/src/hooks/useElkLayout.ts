...
export interface UseElkLayoutParams {
  ...
  /** Notes the user has dragged; read as the ELK interactive-mode seed. */
 ...
 *
 * plus the "seed new nodes next to the note that was just acted on" anchoring
 * and a viewport fit once the layout is final. Both passes are suspended while a
 * node is being dragged (`dragging`) so a manual move never races a relayout.
 */
export function useElkLayout({
  ...
  // Mirrors `layouted` outside state so a layout pass can read the latest
  // positions without depending on `layouted` - which would re-trigger it.
  const layoutedRef = useRef<LayoutedGraph>(EMPTY_LAYOUT)
  // The graph the estimated pass last consumed. Lets the pass re-run on drag-end
  // (its `dragging` dep) become a no-op unless the graph actually changed while
  // the drag - and the layout - was suspended.
  ...
  // Pass 1: estimated heights. Runs on every graph change, and once more when a
  // drag ends (its `dragging` dep) - the latter is a no-op unless the graph
  // changed while the drag, and the layout with it, was suspended.
  useEffect(() => {
  ...
  return { layouted, anchorFilename, markInteraction }
}
